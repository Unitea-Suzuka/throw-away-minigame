# ごみすてゲーム(throw-away-minigame)
## 概要・目的
この作品は、幼児向けにデザインされた社会問題をテーマにしたミニゲーム集です。<br>
⽂字を⼀切使⽤せず、イラストや直感的な操作で遊べるため、まだ⽂字が読めない⼦どもでも楽しめます。<br>
⽬的は、幼児期から世界の課題を知るきっかけを作り、将来的に「持続可能な社会」への意識を育むことです。<br>

## 開発環境
- Unity6000.2.6F2
- Visual Studio 2022

## 遊び方
1. スポーンするゴミをドラッグ＆ドロップしてゴミ箱に入れる

<img width="1016" height="694" alt="imageresurt" src="https://github.com/user-attachments/assets/dedc8a8c-6658-40ee-9336-64783455250b" />


2. 一定時間経つとリザルト画面が表示される。スコアは制限時間内に入れたゴミの量によって決まる

<img width="1018" height="700" alt="image-3" src="https://github.com/user-attachments/assets/423c551d-ade0-4079-ad0f-177ead61538c" />

## 外部パッケージ
- [ParticleEffectForUGUI](https://github.com/mobsakai/ParticleEffectForUGUI)
